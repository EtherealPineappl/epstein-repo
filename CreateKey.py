import csv

key = {
    # ============================================================================
    # TRUMP FAMILY & ASSOCIATES
    # ============================================================================
    "Trump": "Trump",
    "Donald Trump": "Trump",
    "President Trump": "Trump",
    "Donald J Trump": "Trump",
    "Ivanka Trump": "Ivanka Trump",
    "Eric Trump": "Eric Trump",
    "Donald Trump Jr": "Don Jr",
    "Don Jr": "Don Jr",
    "Melania Trump": "Melania Trump",
    "Melania": "Melania Trump",
    "Tom Barrack": "Tom Barrack",
    "Thomas Barrack": "Tom Barrack",
    "Rhona Graff": "Rhona Graff",
    "Mar-a-Lago": "Mar-a-Lago",
    "Mar A Lago": "Mar-a-Lago",
    "Trump Plaza": "Trump Plaza",
    "Trump Tower": "Trump Tower",
    "Trump Modeling": "Trump Modeling",
    "Access Hollywood Tape": "Access Hollywood Tape",
    
    # ============================================================================
    # EPSTEIN & MAXWELL
    # ============================================================================
    "Epstein": "Epstein",
    "Jeffrey Epstein": "Epstein",
    "Jeffery Epstein": "Epstein",
    "Jeff Epstein": "Epstein",
    "Maxwell": "Maxwell",
    "Ghislaine": "Maxwell",
    "Ghislaine Maxwell": "Maxwell",
    "Robert Maxwell": "Robert Maxwell",
    "Isabel Maxwell": "Isabel Maxwell",
    
    # ============================================================================
    # EPSTEIN ASSOCIATES & STAFF
    # ============================================================================
    "Jean-Luc Brunel": "Jean-Luc Brunel",
    "Brunel": "Jean-Luc Brunel",
    "Larry Visoski": "Larry Visoski",
    "Visoski": "Larry Visoski",
    "Lesley Groff": "Lesley Groff",
    "Groff": "Lesley Groff",
    "Sarah Kellen": "Sarah Kellen",
    "Kellen": "Sarah Kellen",
    "Nadia Marcinkova": "Nadia Marcinkova",
    "Nadia Marcinko": "Nadia Marcinkova",
    "Marcinkova": "Nadia Marcinkova",
    "Karyna Shuliak": "Karyna Shuliak",
    "Shuliak": "Karyna Shuliak",
    "Shelley Anne Lewis": "Shelley Anne Lewis",
    "Eva Andersson-Dubin": "Eva Andersson-Dubin",
    "Andersson-Dubin": "Eva Andersson-Dubin",
    "Darren Indyke": "Darren Indyke",
    "Indyke": "Darren Indyke",
    "Richard Kahn": "Richard Kahn",
    "Kahn": "Richard Kahn",
    "Alfredo Rodriguez": "Alfredo Rodriguez",
    "Rodriguez": "Alfredo Rodriguez",
    "Rinaldo Rizzo": "Rinaldo Rizzo",
    "Rizzo": "Rinaldo Rizzo",
    
    # ============================================================================
    # VICTIMS & WITNESSES
    # ============================================================================
    "Virginia Giuffre": "Virginia Giuffre",
    "Virginia Roberts": "Virginia Giuffre",
    "Giuffre": "Virginia Giuffre",
    "Roberts": "Virginia Giuffre",
    "Johanna Sjoberg": "Johanna Sjoberg",
    "Sjoberg": "Johanna Sjoberg",
    "Maria Farmer": "Maria Farmer",
    "Annie Farmer": "Annie Farmer",
    "Ashley Massaro": "Ashley Massaro",
    "Chauntae Davies": "Chauntae Davies",
    "Haley Robson": "Haley Robson",
    
    # ============================================================================
    # CLINTON CIRCLE
    # ============================================================================
    "Clinton": "Clinton",
    "Bill Clinton": "Clinton",
    "President Clinton": "Clinton",
    "William J Clinton": "Clinton",
    "Hillary Clinton": "Hillary Clinton",
    "Hillary Rodham Clinton": "Hillary Clinton",
    "Doug Band": "Doug Band",
    "Lisa Belzberg": "Lisa Belzberg",
    
    # ============================================================================
    # BRITISH ROYALTY & POLITICIANS
    # ============================================================================
    "Prince Andrew": "Prince Andrew",
    "Andrew Windsor": "Prince Andrew",
    "Andrew Mountbatten-Windsor": "Prince Andrew",
    "Duke of York": "Prince Andrew",
    "Sarah Ferguson": "Sarah Ferguson",
    "Duchess of York": "Sarah Ferguson",
    "Fergie": "Sarah Ferguson",
    "Amanda Thirsk": "Amanda Thirsk",
    "David Rowland": "David Rowland",
    "Peter Mandelson": "Peter Mandelson",
    "Lord Mandelson": "Peter Mandelson",
    "Reinaldo Avila da Silva": "Reinaldo Avila da Silva",
    "Gordon Brown": "Gordon Brown",
    "Alistair Darling": "Alistair Darling",
    "Nick Butler": "Nick Butler",
    "King Charles III": "King Charles III",
    "Charles III": "King Charles III",
    "Buckingham Palace": "Buckingham Palace",
    "Thames Valley Police": "Thames Valley Police",
    "Metropolitan Police": "Metropolitan Police",
    
    # ============================================================================
    # NORWEGIAN ROYALTY & POLITICIANS
    # ============================================================================
    "Crown Princess Mette-Marit": "Mette-Marit",
    "Mette-Marit": "Mette-Marit",
    "Princess Mette-Marit": "Mette-Marit",
    "Crown Prince Haakon": "Crown Prince Haakon",
    "Terje Rød-Larsen": "Terje Rød-Larsen",
    "Rød-Larsen": "Terje Rød-Larsen",
    "Mona Juul": "Mona Juul",
    "Thorbjørn Jagland": "Thorbjørn Jagland",
    "Jagland": "Thorbjørn Jagland",
    "Børge Brende": "Børge Brende",
    "Brende": "Børge Brende",
    "Jonas Gahr Støre": "Jonas Gahr Støre",
    
    # ============================================================================
    # SWEDISH ROYALTY
    # ============================================================================
    "Princess Sofia": "Princess Sofia",
    "Sofia of Sweden": "Princess Sofia",
    "Duchess of Värmland": "Princess Sofia",
    "Barbro Ehnbom": "Barbro Ehnbom",
    
    # ============================================================================
    # OTHER WORLD LEADERS & POLITICIANS
    # ============================================================================
    "George Bush": "George Bush",
    "George W Bush": "George Bush",
    "George HW Bush": "George Bush",
    "Ehud Barak": "Ehud Barak",
    "Barak": "Ehud Barak",
    "Benjamin Netanyahu": "Benjamin Netanyahu",
    "Netanyahu": "Benjamin Netanyahu",
    "Sara Netanyahu": "Sara Netanyahu",
    "Vladimir Putin": "Vladimir Putin",
    "Putin": "Vladimir Putin",
    "Sergey Lavrov": "Sergey Lavrov",
    "Lavrov": "Sergey Lavrov",
    "Vitaly Churkin": "Vitaly Churkin",
    "Churkin": "Vitaly Churkin",
    "Sergei Belyakov": "Sergei Belyakov",
    "Belyakov": "Sergei Belyakov",
    "Fidel Castro": "Fidel Castro",
    "Castro": "Fidel Castro",
    "Andrés Pastrana": "Andrés Pastrana",
    "Pastrana": "Andrés Pastrana",
    "Robert Mugabe": "Robert Mugabe",
    "Mugabe": "Robert Mugabe",
    "Jacob Zuma": "Jacob Zuma",
    "Zuma": "Jacob Zuma",
    "Karim Wade": "Karim Wade",
    "Abdoulaye Wade": "Abdoulaye Wade",
    "Kevin Rudd": "Kevin Rudd",
    "Rudd": "Kevin Rudd",
    "Andrew Cuomo": "Andrew Cuomo",
    "Cuomo": "Andrew Cuomo",
    "Joe Biden": "Biden",
    "Biden": "Biden",
    "President Biden": "Biden",
    "Narendra Modi": "Narendra Modi",
    "Modi": "Narendra Modi",
    "Hardeep Singh Puri": "Hardeep Singh Puri",
    "Tsakhiagiin Elbegdorj": "Tsakhiagiin Elbegdorj",
    "Elbegdorj": "Tsakhiagiin Elbegdorj",
    "José María Aznar": "José María Aznar",
    "Aznar": "José María Aznar",
    "Ana Botella": "Ana Botella",
    "Miroslav Lajčák": "Miroslav Lajčák",
    "Lajčák": "Miroslav Lajčák",
    "Miroslav Lajcak": "Miroslav Lajčák",
    "Robert Fico": "Robert Fico",
    "John de Jongh": "John de Jongh",
    "Cecile de Jongh": "Cecile de Jongh",
    "Jeanne Brennan Wiebracht": "Jeanne Brennan Wiebracht",
    "Stacey Plaskett": "Stacey Plaskett",
    "George J Mitchell": "George J Mitchell",
    "Mitchell": "George J Mitchell",
    "Mohammed bin Salman": "Mohammed bin Salman",
    "MBS": "Mohammed bin Salman",
    "Sultan Ahmed bin Sulayem": "Sultan Ahmed bin Sulayem",
    "Bin Sulayem": "Sultan Ahmed bin Sulayem",
    "Barack Obama": "Barack Obama",
    "Obama": "Barack Obama",
    
    # ============================================================================
    # TECH BILLIONAIRES & EXECUTIVES
    # ============================================================================
    "Bill Gates": "Bill Gates",
    "Gates": "Bill Gates",
    "Melinda French Gates": "Melinda French Gates",
    "Boris Nikolic": "Boris Nikolic",
    "Nikolic": "Boris Nikolic",
    "Elon Musk": "Elon Musk",
    "Musk": "Elon Musk",
    "Kimbal Musk": "Kimbal Musk",
    "Talulah Riley": "Talulah Riley",
    "Mark Zuckerberg": "Mark Zuckerberg",
    "Zuckerberg": "Mark Zuckerberg",
    "Priscilla Chan": "Priscilla Chan",
    "Peter Thiel": "Peter Thiel",
    "Thiel": "Peter Thiel",
    "Palantir": "Palantir",
    "Sergey Brin": "Sergey Brin",
    "Brin": "Sergey Brin",
    "Eric Schmidt": "Eric Schmidt",
    "Schmidt": "Eric Schmidt",
    "Reid Hoffman": "Reid Hoffman",
    "Hoffman": "Reid Hoffman",
    "LinkedIn": "Reid Hoffman",
    "Nathan Myhrvold": "Nathan Myhrvold",
    "Myhrvold": "Nathan Myhrvold",
    "Intellectual Ventures": "Intellectual Ventures",
    "Larry Ellison": "Larry Ellison",
    "Ellison": "Larry Ellison",
    
    # ============================================================================
    # WALL STREET & FINANCE
    # ============================================================================
    "Les Wexner": "Les Wexner",
    "Leslie Wexner": "Les Wexner",
    "Wexner": "Les Wexner",
    "Abigail Wexner": "Abigail Wexner",
    "Leon Black": "Leon Black",
    "Black": "Leon Black",
    "Apollo Global Management": "Apollo Global Management",
    "Marc Rowan": "Marc Rowan",
    "Rowan": "Marc Rowan",
    "Sanjay Patel": "Sanjay Patel",
    "Josh Harris": "Josh Harris",
    "Harris": "Josh Harris",
    "Harris Blitzer Sports": "Harris Blitzer Sports",
    "Glenn Dubin": "Glenn Dubin",
    "Dubin": "Glenn Dubin",
    "Jes Staley": "Jes Staley",
    "Staley": "Jes Staley",
    "Barclays": "Barclays",
    "JPMorgan Chase": "JPMorgan Chase",
    "Jamie Dimon": "Jamie Dimon",
    "Dimon": "Jamie Dimon",
    "Ariane de Rothschild": "Ariane de Rothschild",
    "Edmond de Rothschild": "Edmond de Rothschild",
    "Lynn Forester de Rothschild": "Lynn Forester de Rothschild",
    "Andrew Stein": "Andrew Stein",
    "Ronald Lauder": "Ronald Lauder",
    "Lauder": "Ronald Lauder",
    "Howard Lutnick": "Howard Lutnick",
    "Lutnick": "Howard Lutnick",
    "Allison Lutnick": "Allison Lutnick",
    "Cantor Fitzgerald": "Cantor Fitzgerald",
    "Newmark": "Newmark",
    "Thomas Pritzker": "Thomas Pritzker",
    "Tom Pritzker": "Thomas Pritzker",
    "Pritzker": "Thomas Pritzker",
    "Penny Pritzker": "Penny Pritzker",
    "Hyatt Hotels": "Hyatt Hotels",
    "Todd Boehly": "Todd Boehly",
    "Boehly": "Todd Boehly",
    "Guggenheim Partners": "Guggenheim Partners",
    "Chelsea FC": "Chelsea FC",
    "Anil Ambani": "Anil Ambani",
    "Ambani": "Anil Ambani",
    "Andrew Farkas": "Andrew Farkas",
    "Farkas": "Andrew Farkas",
    "Guy Laliberté": "Guy Laliberté",
    "Laliberté": "Guy Laliberté",
    "Cirque du Soleil": "Cirque du Soleil",
    "One Drop Foundation": "One Drop Foundation",
    "Ron Baron": "Ron Baron",
    "Baron": "Ron Baron",
    "Robert Kraft": "Robert Kraft",
    "Kraft": "Robert Kraft",
    "John Havens": "John Havens",
    "Viktor Vekselberg": "Viktor Vekselberg",
    "Vekselberg": "Viktor Vekselberg",
    "Eduardo Teodorani": "Eduardo Teodorani",
    
    # ============================================================================
    # LAWYERS & LEGAL
    # ============================================================================
    "Alan Dershowitz": "Dershowitz",
    "Dershowitz": "Dershowitz",
    "Bob Shapiro": "Bob Shapiro",
    "Robert Shapiro": "Bob Shapiro",
    "Shapiro": "Bob Shapiro",
    "Ken Starr": "Ken Starr",
    "Starr": "Ken Starr",
    "Brad Karp": "Brad Karp",
    "Karp": "Brad Karp",
    "Paul Weiss": "Paul Weiss",
    "Kathryn Ruemmler": "Kathryn Ruemmler",
    "Ruemmler": "Kathryn Ruemmler",
    "Goldman Sachs": "Goldman Sachs",
    "David Schoen": "David Schoen",
    "Schoen": "David Schoen",
    "Brett Kavanaugh": "Brett Kavanaugh",
    "Kavanaugh": "Brett Kavanaugh",
    "Christine Blasey Ford": "Christine Blasey Ford",
    "Reid Weingarten": "Reid Weingarten",
    "Roy Black": "Roy Black",
    "Gerald Lefcourt": "Gerald Lefcourt",
    "Steptoe & Johnson": "Steptoe & Johnson",
    "Latham & Watkins": "Latham & Watkins",
    
    # ============================================================================
    # ACADEMICS & SCIENTISTS
    # ============================================================================
    "Noam Chomsky": "Noam Chomsky",
    "Chomsky": "Noam Chomsky",
    "Valeria Chomsky": "Valeria Chomsky",
    "Stephen Hawking": "Stephen Hawking",
    "Hawking": "Stephen Hawking",
    "Steven Pinker": "Steven Pinker",
    "Pinker": "Steven Pinker",
    "Lawrence Krauss": "Lawrence Krauss",
    "Krauss": "Lawrence Krauss",
    "Marvin Minsky": "Marvin Minsky",
    "Minsky": "Marvin Minsky",
    "Gloria Rudisch": "Gloria Rudisch",
    "Lisa Randall": "Lisa Randall",
    "Randall": "Lisa Randall",
    "Martin Nowak": "Martin Nowak",
    "Nowak": "Martin Nowak",
    "Program for Evolutionary Dynamics": "Program for Evolutionary Dynamics",
    "George Church": "George Church",
    "Church": "George Church",
    "Stephen Kosslyn": "Stephen Kosslyn",
    "Kosslyn": "Stephen Kosslyn",
    "Dan Ariely": "Dan Ariely",
    "Ariely": "Dan Ariely",
    "Jonathan Haidt": "Jonathan Haidt",
    "Haidt": "Jonathan Haidt",
    "Richard Dawkins": "Richard Dawkins",
    "Dawkins": "Richard Dawkins",
    "Sam Harris": "Sam Harris",
    "Joscha Bach": "Joscha Bach",
    "Bach": "Joscha Bach",
    "MIT Media Lab": "MIT Media Lab",
    "Joi Ito": "Joi Ito",
    "Ito": "Joi Ito",
    "Leon Botstein": "Leon Botstein",
    "Botstein": "Leon Botstein",
    "Bard College": "Bard College",
    "David Gelernter": "David Gelernter",
    "Gelernter": "David Gelernter",
    "Yale": "Yale",
    "Harvard": "Harvard",
    "Harvard University": "Harvard",
    "Harvard Medical School": "Harvard",
    "MIT": "MIT",
    "Stanford": "Stanford",
    "Stanford University": "Stanford",
    "Princeton": "Princeton",
    "Princeton University": "Princeton",
    "Columbia": "Columbia",
    "Columbia University": "Columbia",
    "John Brockman": "John Brockman",
    "Brockman": "John Brockman",
    "Edge Foundation": "Edge Foundation",
    "Elisa New": "Elisa New",
    "Poetry in America": "Poetry in America",
    "Larry Summers": "Larry Summers",
    "Lawrence Summers": "Larry Summers",
    "Summers": "Larry Summers",
    "Robert Trivers": "Robert Trivers",
    "Trivers": "Robert Trivers",
    "Jack Horner": "Jack Horner",
    "Horner": "Jack Horner",
    "Dinochicken Project": "Dinochicken Project",
    "Corina Tarnita": "Corina Tarnita",
    "Tarnita": "Corina Tarnita",
    "Mark Tramo": "Mark Tramo",
    "Tramo": "Mark Tramo",
    "UCLA": "UCLA",
    "Nathan Wolfe": "Nathan Wolfe",
    "Wolfe": "Nathan Wolfe",
    "Edward Boyden": "Edward Boyden",
    "Boyden": "Edward Boyden",
    "Mark Landon": "Mark Landon",
    "Landon": "Mark Landon",
    "Ohio State": "Ohio State",
    "Ohio State University": "Ohio State",
    "Wexner Medical Center": "Wexner Medical Center",
    "Baylor University": "Baylor University",
    "Arizona State University": "Arizona State University",
    "Montana State University": "Montana State University",
    "James Watson": "James Watson",
    "Watson": "James Watson",
    "Richard Feynman": "Richard Feynman",
    "Vladimir Nabokov": "Vladimir Nabokov",
    "Nabokov": "Vladimir Nabokov",
    "Cal Tech": "Cal Tech",
    
    # ============================================================================
    # ARTIFICIAL INTELLIGENCE & ROBOTICS
    # ============================================================================
    "Ben Goertzel": "Ben Goertzel",
    "Goertzel": "Ben Goertzel",
    "David Hanson": "David Hanson",
    "Hanson": "David Hanson",
    "Hanson Robotics": "Hanson Robotics",
    "Sophia Robot": "Sophia Robot",
    "Sophia": "Sophia Robot",
    "Gynoid": "Gynoid",
    "Christopher Poole": "Christopher Poole",
    "Moot": "Christopher Poole",
    "4chan": "4chan",
    "/pol/": "4chan",
    
    # ============================================================================
    # MEDIA & ENTERTAINMENT
    # ============================================================================
    "Woody Allen": "Woody Allen",
    "Allen": "Woody Allen",
    "Soon-Yi Previn": "Soon-Yi Previn",
    "Previn": "Soon-Yi Previn",
    "Michael Jackson": "Michael Jackson",
    "Jackson": "Michael Jackson",
    "Prince Jackson": "Prince Jackson",
    "Paris Jackson": "Paris Jackson",
    "Diana Ross": "Diana Ross",
    "Evan Ross": "Evan Ross",
    "Kevin Spacey": "Kevin Spacey",
    "Spacey": "Kevin Spacey",
    "Bill Cosby": "Bill Cosby",
    "Cosby": "Bill Cosby",
    "Harvey Weinstein": "Harvey Weinstein",
    "Weinstein": "Harvey Weinstein",
    "Brett Ratner": "Brett Ratner",
    "Ratner": "Brett Ratner",
    "David Blaine": "David Blaine",
    "Blaine": "David Blaine",
    "David Copperfield": "David Copperfield",
    "Copperfield": "David Copperfield",
    "Naomi Campbell": "Naomi Campbell",
    "Campbell": "Naomi Campbell",
    "Annabelle Neilson": "Annabelle Neilson",
    "Neilson": "Annabelle Neilson",
    "Casey Wasserman": "Casey Wasserman",
    "Wasserman": "Casey Wasserman",
    "Peggy Siegal": "Peggy Siegal",
    "Siegal": "Peggy Siegal",
    "Michael Wolff": "Michael Wolff",
    "Wolff": "Michael Wolff",
    "Fire and Fury": "Fire and Fury",
    "David Brooks": "David Brooks",
    "Brooks": "David Brooks",
    "New York Times": "New York Times",
    "Edward Jay Epstein": "Edward Jay Epstein",
    "Mira Nair": "Mira Nair",
    "Nair": "Mira Nair",
    "Amelia": "Amelia",
    "Zohran Mamdani": "Zohran Mamdani",
    "Mamdani": "Zohran Mamdani",
    "Gracie Mansion": "Gracie Mansion",
    "Jeff Bezos": "Jeff Bezos",
    "Bezos": "Jeff Bezos",
    "Jay-Z": "Jay-Z",
    "Shawn Carter": "Jay-Z",
    "Pusha T": "Pusha T",
    "Terrence Thornton": "Pusha T",
    "Frédéric Chaslin": "Frédéric Chaslin",
    "Chaslin": "Frédéric Chaslin",
    "Steve Tisch": "Steve Tisch",
    "Tisch": "Steve Tisch",
    "New York Giants": "New York Giants",
    "Jeff Koons": "Jeff Koons",
    "Koons": "Jeff Koons",
    "Andres Serrano": "Andres Serrano",
    "Serrano": "Andres Serrano",
    "David A Ross": "David A Ross",
    "Ross": "David A Ross",
    "Whitney Museum": "Whitney Museum",
    "School of Visual Arts": "School of Visual Arts",
    "Jean Pigozzi": "Jean Pigozzi",
    "Pigozzi": "Jean Pigozzi",
    "Henri Pigozzi": "Henri Pigozzi",
    "Renee Zellweger": "Renee Zellweger",
    "Down with Love": "Down with Love",
    "Forrest Gump": "Forrest Gump",
    "Oprah Winfrey": "Oprah Winfrey",
    "Oprah": "Oprah Winfrey",
    "Piers Morgan": "Piers Morgan",
    "Al Franken": "Al Franken",
    "Anthony Weiner": "Anthony Weiner",
    "Marla Maples": "Marla Maples",
    "Kurt Schwitters": "Kurt Schwitters",
    "David Hamilton": "David Hamilton",
    "Christie's": "Christie's",
    
    # ============================================================================
    # POLITICAL OPERATIVES & STRATEGISTS
    # ============================================================================
    "Steve Bannon": "Steve Bannon",
    "Bannon": "Steve Bannon",
    "Tommy Robinson": "Tommy Robinson",
    "Robinson": "Tommy Robinson",
    "Maria Drokova": "Maria Drokova",
    "Masha Drokova": "Maria Drokova",
    "Drokova": "Maria Drokova",
    "Al Seckel": "Al Seckel",
    "Seckel": "Al Seckel",
    "Joshua Cooper Ramo": "Joshua Cooper Ramo",
    "Ramo": "Joshua Cooper Ramo",
    "Kissinger Associates": "Kissinger Associates",
    "Henry Kissinger": "Henry Kissinger",
    "Kissinger": "Henry Kissinger",
    "Robert F Kennedy Jr": "Robert F Kennedy Jr",
    "RFK Jr": "Robert F Kennedy Jr",
    "Kennedy": "Robert F Kennedy Jr",
    "Mary Richardson Kennedy": "Mary Richardson Kennedy",
    "Pine Ridge Indian Reservation": "Pine Ridge Indian Reservation",
    
    # ============================================================================
    # TRUMP ADMINISTRATION & APPOINTEES
    # ============================================================================
    "Todd Blanche": "Todd Blanche",
    "Blanche": "Todd Blanche",
    "Deputy Attorney General": "Todd Blanche",
    "John Phelan": "John Phelan",
    "Phelan": "John Phelan",
    "Secretary of Navy": "John Phelan",
    "Mehmet Oz": "Mehmet Oz",
    "Dr Oz": "Mehmet Oz",
    "Medicare": "Medicare",
    "Medicaid": "Medicaid",
    "Commerce Secretary": "Howard Lutnick",
    "William Burns": "William Burns",
    "Burns": "William Burns",
    "Director of CIA": "William Burns",
    
    # ============================================================================
    # MEDICAL & HEALTH
    # ============================================================================
    "Peter Attia": "Peter Attia",
    "Attia": "Peter Attia",
    "Deepak Chopra": "Deepak Chopra",
    "Chopra": "Deepak Chopra",
    "Jess Ting": "Jess Ting",
    "Ting": "Jess Ting",
    "Mount Sinai": "Mount Sinai",
    
    # ============================================================================
    # SPORTS & ENTERTAINMENT FIGURES
    # ============================================================================
    "Joe Gibbs": "Joe Gibbs",
    "Gibbs": "Joe Gibbs",
    "Joe Gibbs Racing": "Joe Gibbs Racing",
    "Daniel Snyder": "Daniel Snyder",
    "Snyder": "Daniel Snyder",
    "Brian Vickers": "Brian Vickers",
    "Vickers": "Brian Vickers",
    "NASCAR": "NASCAR",
    "Michael Waltrip": "Michael Waltrip",
    "Waltrip": "Michael Waltrip",
    "Rob Kauffman": "Rob Kauffman",
    "Aaron's": "Aaron's",
    "Sammy Sosa": "Sammy Sosa",
    "Sosa": "Sammy Sosa",
    "Jamie Foxx": "Jamie Foxx",
    "Robin Leach": "Robin Leach",
    "Leach": "Robin Leach",
    "Jean Todt": "Jean Todt",
    "Todt": "Jean Todt",
    "FIA": "FIA",
    "Flavio Briatore": "Flavio Briatore",
    "Briatore": "Flavio Briatore",
    "Richard Branson": "Richard Branson",
    "Branson": "Richard Branson",
    "Virgin Group": "Virgin Group",
    "Dean Kamen": "Dean Kamen",
    "Kamen": "Dean Kamen",
    "Segway": "Segway",
    "iBOT": "iBOT",
    "Beta Technologies": "Beta Technologies",
    "Los Angeles Dodgers": "Los Angeles Dodgers",
    "Los Angeles Lakers": "Los Angeles Lakers",
    "Loews Corporation": "Loews Corporation",
    
    # ============================================================================
    # FRENCH CONNECTIONS
    # ============================================================================
    "Jack Lang": "Jack Lang",
    "Lang": "Jack Lang",
    "Caroline Lang": "Caroline Lang",
    "Arab World Institute": "Arab World Institute",
    "Nicolas Sarkozy": "Nicolas Sarkozy",
    "Sarkozy": "Nicolas Sarkozy",
    "Frédéric Fekkai": "Frédéric Fekkai",
    "Fekkai": "Frédéric Fekkai",
    "Prytanee LLC": "Prytanee LLC",
    
    # ============================================================================
    # UK CONNECTIONS
    # ============================================================================
    "Nick Candy": "Nick Candy",
    "Candy": "Nick Candy",
    "Christian Candy": "Christian Candy",
    "Knightsbridge": "Knightsbridge",
    "Nigel Farage": "Nigel Farage",
    "Farage": "Nigel Farage",
    "Reform UK": "Reform UK",
    "Paul Tweed": "Paul Tweed",
    "Tweed": "Paul Tweed",
    "David Tang": "David Tang",
    "Tang": "David Tang",
    "Shanghai Tang": "Shanghai Tang",
    "ICorrect": "ICorrect",
    "Martha's Vineyard": "Martha's Vineyard",
    "Churchill War Rooms": "Churchill War Rooms",
    "RAF Marham": "RAF Marham",
    "Sandringham": "Sandringham",
    "Evening Standard": "Evening Standard",
    "Downing Street": "Downing Street",
    "10 Downing Street": "10 Downing Street",
    "Ministry of Defence": "Ministry of Defence",
    "Augusta National Golf Club": "Augusta National Golf Club",
    
    # ============================================================================
    # INVESTMENT & INDUSTRY
    # ============================================================================
    "Bobby Kotick": "Bobby Kotick",
    "Kotick": "Bobby Kotick",
    "Activision Blizzard": "Activision Blizzard",
    "Call of Duty": "Call of Duty",
    "Microtransactions": "Microtransactions",
    "Nicole Junkermann": "Nicole Junkermann",
    "Junkermann": "Nicole Junkermann",
    "Mark Lloyd": "Mark Lloyd",
    "David Stern": "David Stern",
    "Stern": "David Stern",
    "Royal Bank of Scotland": "Royal Bank of Scotland",
    "Aston Martin": "Aston Martin",
    "DP World": "DP World",
    
    # ============================================================================
    # RELIGIOUS FIGURES
    # ============================================================================
    "Dalai Lama": "Dalai Lama",
    "Tenzin Gyatso": "Dalai Lama",
    "14th Dalai Lama": "Dalai Lama",
    "Gelug": "Gelug",
    "Tibetan Buddhism": "Tibetan Buddhism",
    "Pope John Paul II": "Pope John Paul II",
    "John Paul II": "Pope John Paul II",
    "Vatican City": "Vatican City",
    "Catholic Church": "Catholic Church",
    
    # ============================================================================
    # JOURNALISTS & MEDIA
    # ============================================================================
    "Julie Brown": "Julie Brown",
    "Miami Herald": "Miami Herald",
    "BuzzFeed News": "BuzzFeed News",
    "Jeffrey Mervis": "Jeffrey Mervis",
    "Science": "Science",
    "Seth Hettena": "Seth Hettena",
    "Huffington Post": "Huffington Post",
    "Wall Street Journal": "Wall Street Journal",
    "Washington Post": "Washington Post",
    "Financial Times": "Financial Times",
    "The Guardian": "The Guardian",
    "The Telegraph": "The Telegraph",
    "The Times": "The Times",
    "Vanity Fair": "Vanity Fair",
    "CNN": "CNN",
    "CBS News": "CBS News",
    "PBS": "PBS",
    "NBC": "NBC",
    "ABC News": "ABC News",
    
    # ============================================================================
    # INVESTIGATORS & PROSECUTORS
    # ============================================================================
    "Brad Edwards": "Brad Edwards",
    "Edwards": "Brad Edwards",
    "Ro Khanna": "Ro Khanna",
    "Khanna": "Ro Khanna",
    "Thomas Massie": "Thomas Massie",
    "Massie": "Thomas Massie",
    "Michael Cohen": "Michael Cohen",
    "Cohen": "Michael Cohen",
    "Distributed Denial of Secrets": "Distributed Denial of Secrets",
    "House Oversight Committee": "House Oversight Committee",
    "Økokrim": "Økokrim",
    
    # ============================================================================
    # VICTIMS/MINORS - SPECIFIC TERMS
    # ============================================================================
    "Underage Girl": "Girls",
    "Underage Girls": "Girls",
    "Minor Girl": "Girls",
    "Minor Girls": "Girls",
    "Teenage Girl": "Girls",
    "Teenage Girls": "Girls",
    "Young Girl": "Girls",
    "Young Girls": "Girls",
    "Young Woman": "Girls",
    "Young Women": "Girls",
    "Underaged": "Girls",
    "Minor": "Girls",
    "Minors": "Girls",
    "Underage Prostitute": "Girls",
    "Underage Prostitutes": "Girls",
    
    # ============================================================================
    # VICTIM TERMS
    # ============================================================================
    "Victim": "Victim",
    "Victims": "Victim",
    "Alleged Victim": "Victim",
    "Purported Victim": "Victim",
    "Accuser": "Victim",
    "Accusers": "Victim",
    "Plaintiff": "Victim",
    "Plaintiffs": "Victim",
    "Complainant": "Victim",
    "Survivor": "Victim",
    "Survivors": "Victim",
    
    # ============================================================================
    # CHILDREN/BABIES
    # ============================================================================
    "Child Victim": "Children",
    "Child Victims": "Children",
    "Child Abuse": "Abuse",
    "Child Sexual Abuse": "Abuse",
    "Child Exploitation": "Abuse",
    "Child Trafficking": "Trafficking",
    "Infant": "Babies",
    "Infants": "Babies",
    "Newborn": "Babies",
    "Newborn Baby": "Babies",
    
    # ============================================================================
    # LOCATIONS - SPECIFIC
    # ============================================================================
    "Little Saint James": "Little Saint James",
    "Little St James": "Little Saint James",
    "Epstein Island": "Little Saint James",
    "Epstein's Island": "Little Saint James",
    "Private Island": "Little Saint James",
    "Caribbean Compound": "Little Saint James",
    "Caribbean Island": "Little Saint James",
    "Zorro Ranch": "Zorro Ranch",
    "New Mexico Ranch": "Zorro Ranch",
    "Palm Beach": "Palm Beach",
    "Palm Beach Estate": "Palm Beach",
    "Palm Beach Mansion": "Palm Beach",
    "Manhattan Townhouse": "Manhattan Townhouse",
    "Epstein's Townhouse": "Manhattan Townhouse",
    "Upper East Side": "Manhattan",
    "East 71st Street": "Manhattan Townhouse",
    "Virgin Islands": "Virgin Islands",
    "US Virgin Islands": "Virgin Islands",
    "USVI": "Virgin Islands",
    "St Thomas": "St Thomas",
    "Saint Thomas": "St Thomas",
    "Rancho Palos Verdes": "Rancho Palos Verdes",
    "Paris Apartment": "Paris",
    "Avenue Foch": "Paris",
    "New York": "New York",
    "Manhattan": "Manhattan",
    "Manhattan Jail": "Manhattan Jail",
    "London": "London",
    "Miami": "Miami",
    "Florida": "Florida",
    "New Mexico": "New Mexico",
    "Santa Fe": "Santa Fe",
    "California": "California",
    "Ibiza": "Ibiza",
    "Spain": "Spain",
    "Cuba": "Cuba",
    "Havana": "Havana",
    "New Albany": "New Albany",
    "Ohio": "Ohio",
    "Rome": "Rome",
    "Italy": "Italy",
    "Palo Alto": "Palo Alto",
    "Long Beach": "Long Beach",
    "SpaceX Factory": "SpaceX Factory",
    "St Barts": "St Barts",
    "Saudi Arabia": "Saudi Arabia",
    "Ritz Hotel London": "Ritz Hotel London",
    "Apollo Theater": "Apollo Theater",
    "DNC": "DNC",
    "Democratic National Committee": "DNC",
    
    # ============================================================================
    # TRAVEL - SPECIFIC
    # ============================================================================
    "Lolita Express": "Flights",
    "Private Jet": "Flights",
    "Private Plane": "Flights",
    "Boeing 727": "Flights",
    "Flight Log": "Flights",
    "Flight Logs": "Flights",
    "Flight Manifest": "Flights",
    "Flight Manifests": "Flights",
    "Private Yacht": "Yacht",
    "Yacht": "Yacht",
    
    # ============================================================================
    # ABUSE TERMS - SPECIFIC
    # ============================================================================
    "Sexual Assault": "Assault",
    "Sexual Abuse": "Abuse",
    "Sexual Abuse of Minors": "Abuse",
    "Sexually Assaulted": "Assault",
    "Sexually Abused": "Abuse",
    "Rape": "Rape",
    "Raped": "Rape",
    "Gang Rape": "Rape",
    "Statutory Rape": "Rape",
    "Forced Sex": "Rape",
    "Non-Consensual": "Rape",
    "Assault": "Assault",
    "Assaulted": "Assault",
    "Molest": "Abuse",
    "Molested": "Abuse",
    "Molestation": "Abuse",
    "Penetration": "Abuse",
    "Penetrated": "Abuse",
    "Oral Sex": "Abuse",
    "Forced Oral": "Abuse",
    "Massage": "Massage",
    "Massages": "Massage",
    "Masseuse": "Massage",
    "Erotic Massage": "Massage",
    "Sexual Massage": "Massage",
    "Orgy": "Orgy",
    "Orgies": "Orgies",
    "Sex Party": "Orgy",
    "Sex Parties": "Orgy",
    "Underage Orgy": "Orgy",
    "Extramarital Sex": "Extramarital Sex",
    "Russian Girls": "Recruitment",
    "Russian Girl": "Recruitment",
    "Ukrainian Girl": "Recruitment",
    "Swedish Blonde": "Recruitment",
    "Tall Swedish Blonde": "Recruitment",
    
    # ============================================================================
    # TRAFFICKING - SPECIFIC
    # ============================================================================
    "Sex Trafficking": "Trafficking",
    "Human Trafficking": "Trafficking",
    "Trafficked": "Trafficking",
    "Recruited": "Recruitment",
    "Groomed": "Recruitment",
    "Grooming": "Recruitment",
    "Procured": "Recruitment",
    "Sex Slave": "Trafficking",
    "Sex Slaves": "Trafficking",
    "Prostitution": "Trafficking",
    "Forced Prostitution": "Trafficking",
    "Soliciting Prostitution": "Legal",
    "Soliciting a Minor": "Legal",
    "Soliciting Underage": "Legal",
    "Handler": "Recruitment",
    "Handlers": "Recruitment",
    
    # ============================================================================
    # VIOLENCE - SPECIFIC
    # ============================================================================
    "Ritual Abuse": "Ritual",
    "Ritualistic Abuse": "Ritual",
    "Satanic Ritual": "Ritual",
    "Sacrifice": "Ritual",
    "Human Sacrifice": "Ritual",
    "Ba'al": "Ba'al",
    "Baal": "Ba'al",
    "Moloch": "Moloch",
    "Molech": "Moloch",
    "Torture": "Torture",
    "Tortured": "Torture",
    "Torture Video": "Torture",
    "Murder": "Murder",
    "Murdered": "Murder",
    "Killed": "Murder",
    "Homicide": "Murder",
    "Strangled": "Murder",
    "Strangulation": "Murder",
    "Missing Person": "Missing",
    "Disappeared": "Missing",
    "Went Missing": "Missing",
    "Threat": "Threats",
    "Threatened": "Threats",
    "Death Threat": "Threats",
    "Blackmail": "Threats",
    "Blackmailed": "Threats",
    "Extortion": "Threats",
    "Extortionists": "Threats",
    
    # ============================================================================
    # DRUGS - SPECIFIC
    # ============================================================================
    "Drugged": "Drugs",
    "Drug-Induced": "Drugs",
    "Sedated": "Drugs",
    "Intoxicated": "Drugs",
    "Rohypnol": "Drugs",
    "Date Rape Drug": "Drugs",
    "Spiked Drink": "Drugs",
    "Cocaine": "Drugs",
    "Ecstasy": "Drugs",
    "MDMA": "Drugs",
    "Nuvigil": "Drugs",
    "Wakefulness Medication": "Drugs",
    
    # ============================================================================
    # LEGAL - SPECIFIC
    # ============================================================================
    "Lawsuit": "Legal",
    "Lawsuits": "Legal",
    "Civil Suit": "Legal",
    "Criminal Case": "Legal",
    "Federal Case": "Legal",
    "Court Case": "Legal",
    "Deposition": "Legal",
    "Testimony": "Legal",
    "Testified": "Legal",
    "Settlement": "Settlement",
    "Settled": "Settlement",
    "Non-Disclosure Agreement": "Legal",
    "NDA": "Legal",
    "Confidentiality Agreement": "Legal",
    "Plea Deal": "Legal",
    "Guilty Plea": "Legal",
    "Pleaded Guilty": "Legal",
    "Indictment": "Legal",
    "Indicted": "Legal",
    "Conviction": "Legal",
    "Convicted": "Legal",
    "Sex Offender": "Legal",
    "Registered Sex Offender": "Legal",
    "Nonprosecution Agreement": "Legal",
    "Non-Prosecution Agreement": "Legal",
    "Suspicious Activity Report": "Legal",
    "Misconduct in Public Office": "Legal",
    "Gross Corruption": "Legal",
    "Diplomatic Immunity": "Legal",
    "Jailhouse Suicide": "Death",
    "Killed Himself": "Death",
    "Suicide": "Death",
    "House Arrest": "Legal",
    "Work Release": "Legal",
    "Jail Term": "Legal",
    "Prison Sentence": "Legal",
    "Serving Time": "Legal",
    
    # ============================================================================
    # ORGANIZATIONS - SPECIFIC
    # ============================================================================
    "Victoria's Secret": "Victoria's Secret",
    "VS Models": "Victoria's Secret",
    "Edward Razek": "Edward Razek",
    "Modeling Agency": "Modeling",
    "Model Agency": "Modeling",
    "MC2 Model Management": "Modeling",
    "Teen Models": "Modeling",
    "Modeling Scout": "Modeling",
    "CIA": "CIA",
    "Central Intelligence Agency": "CIA",
    "FBI": "FBI",
    "Federal Bureau of Investigation": "FBI",
    "DOJ": "DOJ",
    "Department of Justice": "DOJ",
    "Justice Department": "DOJ",
    "Mossad": "Israel",
    "Israeli Intelligence": "Israel",
    "International Peace Institute": "International Peace Institute",
    "IPI": "International Peace Institute",
    "Council of Europe": "Council of Europe",
    "World Economic Forum": "World Economic Forum",
    "Davos": "World Economic Forum",
    "Nobel Committee": "Nobel Committee",
    "Clinton Foundation": "Clinton Foundation",
    "Southern Trust Company": "Southern Trust Company",
    "Southern Trust Co": "Southern Trust Company",
    "Financial Trust Company": "Financial Trust Company",
    "Hyperion Air": "Hyperion Air",
    "Liquid Funding": "Liquid Funding",
    "Gratitude America": "Gratitude America",
    "MacArthur Foundation": "MacArthur Foundation",
    "Knight Foundation": "Knight Foundation",
    "Bill and Melinda Gates Foundation": "Bill and Melinda Gates Foundation",
    "OpenAI": "OpenAI",
    "Center for American Progress": "Center for American Progress",
    "Budget Lab at Yale": "Budget Lab at Yale",
    "Union College": "Union College",
    "Burning Man": "Burning Man",
    "Burning Man Project": "Burning Man Project",
    
    # ============================================================================
    # MONEY - SPECIFIC
    # ============================================================================
    "Hush Money": "Money",
    "Payoff": "Money",
    "Bribe": "Money",
    "Bribed": "Money",
    "Financial Settlement": "Settlement",
    "Wire Transfer": "Money",
    "Cash Payment": "Money",
    "Offshore Account": "Money",
    "Shell Company": "Money",
    "Tax Fraud": "Money",
    "Money Laundering": "Money",
    "Wealth Management": "Money",
    "Personal Wealth Management": "Money",
    "Tax Planning": "Money",
    "Estate Planning": "Money",
    "Tax Inversion": "Money",
    "Tax Receivable Agreement": "Money",
    "Financial Planner": "Money",
    
    # ============================================================================
    # FINANCIAL CRISIS
    # ============================================================================
    "2008 Crisis": "2008 Crisis",
    "Financial Crisis": "Financial Crisis",
    "Market Crash": "Financial Crisis",
    "Economic Collapse": "Financial Crisis",
    "Trilateral Commission": "Trilateral Commission",
    "Federal Reserve": "Federal Reserve",
    "Jerome Powell": "Jerome Powell",
    "Powell": "Jerome Powell",
    "Euro Bailout": "Euro Bailout",
    "Eurozone Crisis": "Euro Bailout",
    "Bankers Bonus Tax": "Bankers Bonus Tax",
    "Bank Trading Restrictions": "Bank Trading Restrictions",
    "Gilts": "Gilts",
    
    # ============================================================================
    # CRYPTO
    # ============================================================================
    "Bitcoin": "Bitcoin",
    "Cryptocurrency": "Cryptocurrency",
    "Crypto Currency": "Cryptocurrency",
    "Blockchain": "Cryptocurrency",
    
    # ============================================================================
    # SOCIAL/POLITICAL
    # ============================================================================
    "Homeless": "Homeless",
    "Homelessness": "Homeless",
    "Abortion": "Abortion",
    "Gene Pool": "Gene Pool",
    "Eugenics": "Eugenics",
    "Eugenic": "Eugenics",
    "Genetic Selection": "Eugenics",
    "MeToo": "MeToo",
    "Me Too": "MeToo",
    "#MeToo": "MeToo",
    "Witch Hunt": "Witch Hunt",
    
    # ============================================================================
    # CONSPIRACY TERMS
    # ============================================================================
    "Pizzagate": "Pizza",
    "Calendar Girls": "Calendar Girls",
    "Snow White": "Snow White",
    
    # ============================================================================
    # DOCUMENTS & RECORDS
    # ============================================================================
    "Epstein Files": "Epstein Files",
    "Document Release": "Document Release",
    "FBI Report": "FBI Report",
    "FBI Tips": "FBI Tips",
    "Crisis Intake Form": "Crisis Intake Form",
    "Tip Line": "Tip Line",
    "Black Book": "Black Book",
    "Birthday Book": "Birthday Book",
    "Email Chains": "Email Chains",
    "Text Messages": "Text Messages",
    "Scheduling Documents": "Scheduling Documents",
    "Calendar": "Calendar",
    "Phone Message Pads": "Phone Message Pads",
    "Mug Shot": "Mug Shot",
    "Redacted": "Redacted",
    "Unredacted": "Unredacted",
    "Personal Identifying Information": "Personal Identifying Information",
    
    # ============================================================================
    # MISCELLANEOUS TERMS
    # ============================================================================
    "Lolita": "Lolita",
    "Gang Stuff": "Gang Stuff",
    "Illicit Trysts": "Illicit Trysts",
    "Sluty Spanish Girl": "Recruitment",
    "Hottie Interns": "Hottie Interns",
    "Female Viagra": "Female Viagra",
    "Sexual Behavior Research": "Sexual Behavior Research",
    "Sex Drive": "Sex Drive",
    "Great Friend": "Great Friend",
    "Best Friend": "Best Friend",
    "Trusted Money Man": "Money",
    "Wing Man": "Recruitment",
    "Mr Big": "Mr Big",
    "Harem": "Harem",
    "Pro or Civilian": "Recruitment",
    "Tight Leather Outfit": "Inappropriate",
    "Ratio": "Ratio",
    "Wildest Party": "Wildest Party",
    "Party Scene": "Party Scene",
    "Red Head": "Recruitment",
    "Redhead": "Recruitment",
    "Great Girl": "Recruitment",
    "Beautiful Russian Model": "Recruitment",
    "26-year-old Russian Woman": "Recruitment",
    "Adult Women": "Adult Women",
    "Awfully Coy": "Awfully Coy",
    "Hung Like a Horse": "Inappropriate",
    "Statutory": "Statutory",
    "Juvenal Mug Shots": "Juvenal Mug Shots",
    "Sell-By Date": "Sell-By Date",
    "Past Their Sell-By Date": "Sell-By Date",
    "Find Some Girls": "Recruitment",
    "Find Girls": "Recruitment",
    "Bring Girls": "Recruitment",
    "Russian Intelligence Asset": "Russian Intelligence Asset",
    "Spy": "Spy",
    "Mossad Spy": "Spy",
    "Image Rehabilitation": "Image Rehabilitation",
    "Reputation Management": "Reputation Management",
    "Search Engine Optimization": "Search Engine Optimization",
    "SEO": "Search Engine Optimization",
    "Wikipedia Editing": "Wikipedia Editing",
    "Media Strategy": "Media Strategy",
    "Fake Images": "Fake Images",
    "Fake Documents": "Fake Documents",
    "Fake Videos": "Fake Videos",
    "AI-Generated Photos": "AI-Generated Photos",
    "Uncorroborated": "Uncorroborated",
    "Unverified": "Unverified",
    "False Claims": "False Claims",
    "Fantastical Scenarios": "Fantastical Scenarios",
    
    # ============================================================================
    # EASTER EGG
    # ============================================================================
    "Freddy Fazbear": "Freddy Fazbear",
    "Five Nights at Freddy's": "Freddy Fazbear",
    "FNAF": "Freddy Fazbear",
}

# Write to CSV
with open('word_categories.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
    writer.writerow(['Word', 'Category'])
    for word, category in key.items():
        writer.writerow([word, category])

print("CSV created: word_categories.csv")
print(f"Total entries: {len(key)}")